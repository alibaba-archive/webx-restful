...
public class ReflectionUtils {
    ...
    /**
     * Get the declaring class of an accessible object. Supported are {@link Method}, {@link Field} and
     * {@link Constructor} accessible object types.
     * 
     * @param ao an accessible object.
     * @return the declaring class of an accessible object.
     * @throws IllegalArgumentException in case the type of the accessible object is not supported.
     */
     ...
     * <p>
     * Returns a string consisting of the name of the class of which the object is an instance, the at-sign character '
     * <code>@</code>', the unsigned hexadecimal representation of the hash code of the object, the character '
     * <code>.</code>', the name of the method, the character '<code>(</code>', the list of method parameters, and the
     * character '<code>)</code>'. In other words, thos method returns a string equal to the value of: <blockquote>
     * 
     * <pre>
     * o.getClass().getName() + '@' + Integer.toHexString(o.hashCode()) +
     ...
     * <p>
     * The context class loader will be utilized if accessible and non-null. Otherwise the defining class loader of this
        ...
        if (cl != null) {
            try {
                ...
                if (LOGGER.isLoggable(Level.FINE)) {
                    LOGGER.log(Level.FINE, "Unable to load class " + name + " using the supplied classloader "
    ...
    /**
     * Get the list of classes that represent the type arguments of a {@link ParameterizedType parameterized} input
     * type.
     * <p />
     ...
     * <li>If the type argument is a generic array type and the generic component type is a class then class of the
     * array is returned.</li>
     * <li>If the type argument is a parameterized type and it's raw type is a class then that class is returned.</li>
     * </ul>
     * If the {@code type} is not an instance of ParameterizedType an empty list is returned.
     * 
     * @param type parameterized type.
     * @return the list of classed representing the actual type arguments. May be empty, but may never be {@code null}.
     * @throws IllegalArgumentException if any of the generic type arguments is not a class, or a generic array type, or
     * the generic component type of the generic array type is not class, or not a parameterized type with a raw type
     * that is not a class.
     */
    ...
    /**
     * Get the list of class-type pairs that represent the type arguments of a {@link ParameterizedType parameterized}
     * input type.
     * <p />
     * For any given {@link ClassTypePair#rawClass() class} part of each pair in the returned list, following rules
     * apply:
     * <ul>
     * <li>If a type argument is a class then the class is returned as raw class.</li>
     * <li>If the type argument is a generic array type and the generic component type is a class then class of the
     * array is returned as raw class.</li>
     * <li>If the type argument is a parameterized type and it's raw type is a class then that class is returned as raw
     * class.</li>
     * </ul>
     * If the {@code type} is not an instance of ParameterizedType an empty list is returned.
     * 
     * @param type parameterized type.
     * @return the list of class-type pairs representing the actual type arguments. May be empty, but may never be
     * {@code null}.
     * @throws IllegalArgumentException if any of the generic type arguments is not a class, or a generic array type, or
     * the generic component type of the generic array type is not class, or not a parameterized type with a raw type
     * that is not a class.
    ...
    /**
     * Resolve generic type parameter(s) of a raw class and it's generic type based on the class that declares the
     * generic type parameter(s) to be resolved and a concrete implementation of the declaring class.
     * 
     * @param concreteClass concrete implementation of the declaring class.
     * @param declaringClass class declaring the generic type parameter(s) to be resolved.
     ...
     * @return a pair of class and the generic type values with the the resolved generic parameter types.
     */
    public static ClassTypePair resolveGenericType(final Class<?> concreteClass, final Class<?> declaringClass,
        ...
        if (genericResolvedType instanceof TypeVariable) {
            ...
            final ClassTypePair ct = resolveGenericType(concreteClass, declaringClass, null,
                                                        gat.getGenericComponentType());
            if (gat.getGenericComponentType() != ct.type()) {
                try {
                    Class<?> ac = ReflectionUtils.getArrayClass(ct.rawClass());
     ...
     * <p>
     * If there exists a public method on the class that has the same name and parameters as the existing method then
     * that public method is returned.
     * <p>
     * Otherwise, if there exists a public method on the class that has the same name and the same number of parameters
     * as the existing method, and each generic parameter type, in order, of the public method is equal to the generic
     * parameter type, in the same order, of the existing method or is an instance of {@link TypeVariable} then that
     * public method is returned.
     * 
